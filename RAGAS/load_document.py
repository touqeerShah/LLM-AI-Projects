...
for document in data:
    paragraph_text = document.get('paragraphsData', {}).get('paragraph', "")
    headers = document.get('headerData', [])  # Assuming headerData is always present; adjust similarly if not
    
    # Combine paragraph text with headers, making sure to skip if paragraph_text is empty or a placeholder
    texts_to_process = [paragraph_text] + headers if paragraph_text else headers
    if count==1000:
            break;
